...
public class ScheduleManagerPanel extends BaseManagerPanel {
    ...
    protected void initComponents() {
        ...
        for (String d : days) {
            ...
            lbl.setBorder(new EmptyBorder(10,0,10,0)); pnlDaysHeader.add(lbl);
        }
        ...
        pnlForm.setBackground(Color.WHITE); pnlForm.setPreferredSize(new Dimension(400, 0));
        
        pnlForm.add(new JLabel("THÔNG TIN BUỔI HỌC"), "center, gapy 10");
        pnlForm.add(createFieldLabel("Lớp học:")); cmbClass = new JComboBox<>(); pnlForm.add(cmbClass, "height 35");
        ...
        pnlForm.add(createFieldLabel("Phòng học:")); cmbRoom = new JComboBox<>(); pnlForm.add(cmbRoom, "height 35");
    ...
    protected void loadTableData() {
        ...
        lblWeekRange.setText(String.format("Tuần: %s - %s", currentMonday.format(dateFormatter), sunday.format(dateFormatter)));
        ...
        week.forEach(s -> tableModel.addRow(new Object[]{ 
            s.getSchedule_id(), s.getClazz().getClassName(), s.getStudyDate(), s.getStartTime(), s.getEndTime(), s.getRoom() != null ? s.getRoom().getRoomName() : "N/A" 
        }));
    ...
    private void updateCalendarGrid(List<Schedule> list) {
        ...
        Map<Integer, List<Schedule>> byDay = list.stream().collect(Collectors.groupingBy(s -> s.getStudyDate().getDayOfWeek().getValue()));
        for (int i = 1; i <= 7; i++) {
            ...
            lblDate.setFont(new Font("Segoe UI", Font.ITALIC, 11)); lblDate.setForeground(Color.GRAY);
    ...
    private JPanel createCard(Schedule s) {
        ...
        lblTime.setFont(new Font("Segoe UI", Font.BOLD, 12)); lblTime.setForeground(UIHelper.PRIMARY_COLOR);
        c.add(lblTime);
        JLabel lblName = new JLabel("<html><body style='width: 80px'>" + s.getClazz().getClassName() + "</body></html>");
        lblName.setFont(new Font("Segoe UI", Font.BOLD, 12)); c.add(lblName, "growx");
        return c;
    }

    @Override 
    protected void handleEvents() { 
        btnPrevWeek.addActionListener(e -> { currentMonday = currentMonday.minusWeeks(1); loadTableData(); });
        btnNextWeek.addActionListener(e -> { currentMonday = currentMonday.plusWeeks(1); loadTableData(); });
        btnCurrentWeek.addActionListener(e -> { currentMonday = LocalDate.now().with(TemporalAdjusters.previousOrSame(DayOfWeek.MONDAY)); loadTableData(); });
        dpFilter.addDateChangeListener(e -> { if (e.getNewDate() != null) { currentMonday = e.getNewDate().with(TemporalAdjusters.previousOrSame(DayOfWeek.MONDAY)); loadTableData(); } });

        btnClear.addActionListener(e -> { dpStudyDate.clear(); tpStartTime.clear(); tpEndTime.clear(); tblSchedule.clearSelection(); }); 
        btnAdd.addActionListener(e -> {
            Schedule s = getScheduleFromFields();
            if (s != null) { JOptionPane.showMessageDialog(this, scheduleController.createSchedule(s)); loadTableData(); }
        });
        btnUpdate.addActionListener(e -> {
            int row = tblSchedule.getSelectedRow();
            if (row < 0) { JOptionPane.showMessageDialog(this, "Chọn lịch học cần sửa!"); return; }
            Schedule s = getScheduleFromFields();
            if (s != null) { s.setSchedule_id((Long) tableModel.getValueAt(row, 0)); JOptionPane.showMessageDialog(this, scheduleController.updateSchedule(s)); loadTableData(); }
        });
        btnDelete.addActionListener(e -> {
            int row = tblSchedule.getSelectedRow();
            if (row < 0) { JOptionPane.showMessageDialog(this, "Chọn lịch học cần xóa!"); return; }
            if (JOptionPane.showConfirmDialog(this, "Xác nhận xóa?", "Xác nhận", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                scheduleController.deleteSchedule(((Long) tableModel.getValueAt(row, 0)).intValue()); loadTableData();
            }
        });
        tblSchedule.getSelectionModel().addListSelectionListener(e -> { if (!e.getValueIsAdjusting()) fillFieldsFromSelectedRow(); });
    }
    ...
    private void fillFieldsFromSelectedRow() {
        ...
        if (row >= 0) {
            ...
            if (s != null) { cmbClass.setSelectedItem(s.getClazz()); dpStudyDate.setDate(s.getStudyDate()); tpStartTime.setTime(s.getStartTime()); tpEndTime.setTime(s.getEndTime()); cmbRoom.setSelectedItem(s.getRoom()); }
        }
    }
}
