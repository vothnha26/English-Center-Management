...
public class ScheduleManagerFrame extends BaseManagerFrame {
    ...
    private void deleteSchedule() {
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            JOptionPane.showMessageDialog(this,
                    scheduleController.deleteSchedule(selectedSchedule.getSchedule_id().intValue()));
            loadTableData();
            clearForm();
        }
    }
    ...
    private void filterByClass() {
        ...
        if (c != null)
            renderTable(scheduleController.getSchedulesByClass(c));
    }

    private void filterByRoom() {
        Room r = (Room) cmbRoom.getSelectedItem();
        if (r != null)
            renderTable(scheduleController.getSchedulesByRoom(r));
    }

    @Override
    protected void loadTableData() {
        if (scheduleController != null)
            renderTable(scheduleController.getAllSchedules());
    }
    ...
    private void renderTable(List<Schedule> list) {
        ...
        for (Schedule s : list) {
            tableModel.addRow(new Object[] {
                    ...
                    s.getClazz() != null && s.getClazz().getCourse() != null ? s.getClazz().getCourse().getCourseName()
                            : "N/A",
                    s.getStudyDate(),
                    s.getStartTime(),
                    s.getEndTime(),
