...
public class AttendanceManagerFrame extends BaseManagerFrame {
    ...
    private void deleteAttendance() {
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            JOptionPane.showMessageDialog(this,
                    attendanceController.deleteAttendance(selectedAttendance.getAttendance_id().intValue()));
            loadTableData();
            clearForm();
        }
    }
    ...
    private void filterByClass() {
        ...
        if (c != null)
            renderTable(attendanceController.getAttendancesByClass(c));
    }
    ...
    private void showStatistics() {
        ...
        if (c == null)
            return;
        String start = JOptionPane.showInputDialog(this, "Từ ngày (yyyy-MM-dd):",
        ...
        if (start != null && end != null) {
            try {
                ...
                JOptionPane.showMessageDialog(this,
                        String.format("Tỷ lệ có mặt lớp %s: %.2f%%", c.getClassName(), rate));
            } catch (Exception ex) {
    ...
    protected void loadTableData() {
        if (attendanceController != null)
            renderTable(attendanceController.getAllAttendances());
    }
    ...
    private void renderTable(List<Attendance> list) {
        ...
        for (Attendance a : list) {
            tableModel.addRow(new Object[] {
                    ...
                    a.getClazz() != null && a.getClazz().getCourse() != null ? a.getClazz().getCourse().getCourseName()
                            : "N/A",
                    a.getAttend_date(),
