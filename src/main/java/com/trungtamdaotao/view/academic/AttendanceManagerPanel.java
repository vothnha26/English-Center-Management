...
public class AttendanceManagerPanel extends BaseManagerPanel {
    ...
    private JPanel createChecklistTab() {
        ...
        checkModel = new DefaultTableModel(cols, 0) {
            @Override public Class<?> getColumnClass(int c) { return c == 0 ? AttendanceStatus.class : String.class; }
    ...
    protected void handleEvents() {
        ...
        btnLoadSummary.addActionListener(e -> {
            ClassEntity clazz = (ClassEntity) cmbSummaryClass.getSelectedItem();
            if (clazz == null) { JOptionPane.showMessageDialog(this, "Vui lòng chọn lớp học!"); return; }
            loadSummaryData(clazz);
        });
    }
